...
def list_projects(manager: TodoManager) -> None:
...
def edit_project(manager: TodoManager) -> None:
    """Edit a project"""
    try:
        print("\n=== Edit Project ===")
...
def delete_project(manager: TodoManager) -> None:
    """Delete a project"""
    try:
        print("\n=== Delete Project ===")
        ...
        # Confirm deletion
        confirm = input(
            f"⚠️ Are you sure you want to delete project '{selected_project.name}' and all its {len(selected_project.tasks)} tasks? (y/N): "
        ).strip().lower()
