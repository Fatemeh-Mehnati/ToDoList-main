...
def create_task(manager: TodoManager) -> None:
    """Create a new task"""
    try:
        print("\n=== Create New Task ===")
...
def list_tasks(manager: TodoManager) -> None:
    ...
    try:
        ...
        for i, task in enumerate(tasks, 1):
            ...
            if task.deadline:
                # type: ignore[attr-defined] - deadline is Optional[datetime]
...
def edit_task(manager: TodoManager) -> None:
    """Edit a task"""
    try:
        print("\n=== Edit Task ===")
        ...
        if title or description or status or deadline:
            updated_task = manager.edit_task(
                task_id, title=title, description=description, status=status, deadline=deadline
            )
...
def change_task_status(manager: TodoManager) -> None:
    """Change task status"""
    try:
        print("\n=== Change Task Status ===")
